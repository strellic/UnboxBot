...
const mergeImages = (sources = [], options = {}) => new Promise(resolve => {
	...
	const images = sources.map(source => new Promise((resolve, reject) => {
		...
		img.onerror = () => {
			console.log("[ERR] Couldn't load image @ mergeImages: ", source);

			let placeholder = new Image();
			placeholder.crossOrigin = options.crossOrigin;
			placeholder.onerror = () => {
				console.log("[ERR] Couldn't load placeholder @ mergeImages!");
			};
			placeholder.onload = () => resolve(Object.assign({}, source, { placeholder }));
